...
形名参同は、AI の自己申告を機械的な証拠へ置き換えるための手法であり、品質保証そのものを完全自動化するものではない。特に「名」自体が間違っている場合、その間違いを検出するには別のレビュー観点が必要になる。

### 防げること

| 防げること | 理由 |
|---|---|
| 「動きました」という証拠なしの完了報告 | コマンド出力・終了コード・テスト結果を「形」として要求するため |
| 実装後に成功基準を緩める後付け正当化 | 「名」を実装前に固定するため |
| typecheck / lint / unit / E2E などの実行漏れ | `ai:check` のような統合入口で一括実行できるため |
| ローカルと CI で異なる検証を走らせるずれ | 同じ npm scripts を shell entrypoint / hook / CI から呼べるため |
| AI が一部だけ確認して全体完了と見なす誤認 | 照合表で未実行・不一致・未解決リスクを分離できるため |

### 防げないこと

| 防げないこと | なぜ防げないか |
|---|---|
| 仕様や受け入れ条件そのものの誤り | 間違った「名」を正しく満たしても、成果物は正しいとは限らない |
| テストが存在しない経路の不具合 | 「名」に含まれていない観点は照合対象にならない |
| テストが誤った期待値を assert している問題 | 終了コード 0 はテストの意味的妥当性を保証しない |
| UI/UX、文章、運用判断などの人間の価値判断 | 多くは機械的な pass/fail に落とし切れない |
| 脅威モデル外のセキュリティ・プライバシーリスク | scanner や review checklist に入っていないリスクは残る |
| flaky test や外部サービス依存による偶発的 pass/fail | 測定環境の信頼性は別途管理が必要 |

### 補完策

| 限界 | 補完策 |
|---|---|
| 仕様が間違っている | 実装前に人間が acceptance criteria と non-goals をレビューする |
| テスト網羅性が足りない | [`test-pyramid.md`](./test-pyramid.md) と [`qa-techniques.md`](./qa-techniques.md) で観点を増やす |
| 意味的正しさが不十分 | Given-When-Then で主要シナリオを明文化し、コードレビューで仕様との対応を確認する |
| セキュリティ観点が薄い | `ai:check:secure`、権限最小化、secret redaction、threat review を別 gate として足す |
| 人間の判断が必要 | 「形」は受け入れ判断の材料であり、最終承認は人間が行う |

このため、形名参同の実務上の位置づけは「最終判断を置き換える仕組み」ではなく、**人間が判断する前に、最低限の証拠を揃える仕組み**である。
